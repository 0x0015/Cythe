...
void AssetLoader::LoadImages(){
	for(auto it : Images){
		if(it.second == 0){
			...
			Logger::log("Loading Image:  ",it.first," with ",Images[it.first]);
		}
	}
}
void AssetLoader::LoadAudio(){
	for(auto it : Audios){
		if(it.second == 0){
			...
			Logger::log("Loading Audio:  ",it.first," with ",Audios[it.first]);
		}
	}
}
void AssetLoader::LoadFonts(){
	for(auto it : Fonts){
		if(it.second == 0){
			...
			Logger::log("Loading Font:  ",it.first.first," size ",it.first.second," with ",Fonts[it.first]);
		}
	}
}
...
void AssetLoader::LoadShaders(){
	for(auto it : Shaders){
		if(it.second == 0){
			...
			Logger::log("Loading Shader  frag:",it.first.first," vert:",it.first.second," with ",Shaders[it.first]);
		}
	}
}
void AssetLoader::LoadComputeShaders(){
	for(auto it : ComputeShaders){
		if(it.second == 0){
			...
			Logger::log("Loading Compute shader:   ",it.first," with ",ComputeShaders[it.first]);
		}
	}
}
